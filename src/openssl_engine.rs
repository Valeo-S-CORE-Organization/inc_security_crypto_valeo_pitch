...
use crate::traits::{CryptoEngine, StreamHasher};
...
impl CryptoEngine for OpenSslEngine {
    ...
    fn generate_rsa_key_pair(&self, bits: u32) -> Result<RsaKeyPair, CryptoError> {
        ...
        let private_der = pkey.private_key_to_pkcs8().map_err(key_err)?;
    ...
    fn generate_aes_key(&self, len: usize) -> Result<Vec<u8>, CryptoError> {
        ...
        rand_bytes(&mut key).map_err(key_err)?;
        Ok(key)
    }
    ...
    fn aes_gcm_decrypt(
        ...
        let cipher = aes_gcm_cipher(key.len())?;
        decrypt_aead(cipher, key, Some(iv), aad, ciphertext, tag).map_err(decrypt_err)
    }
    ...
    fn rsa_pkcs1_encrypt(
        ...
        let pkey = PKey::<Public>::public_key_from_der(public_der).map_err(invalid_key_err)?;
    ...
    fn rsa_pkcs1_decrypt(
        ...
        let pkey = PKey::<Private>::private_key_from_pkcs8(private_der).map_err(invalid_key_err)?;
        let rsa  = pkey.rsa().map_err(invalid_key_err)?;
        let mut out = vec![0u8; rsa.size() as usize];
    ...
    fn rsa_pkcs1_sign(
        ...
        let pkey = PKey::private_key_from_pkcs8(private_der).map_err(invalid_key_err)?;
    ...
    fn rsa_pkcs1_verify(
        ...
        let pkey = PKey::public_key_from_der(public_der).map_err(invalid_key_err)?;
    ...
    fn aes_key_wrap(
        ...
        let aes_key = AesKey::new_encrypt(kek)
    ...
    fn aes_key_unwrap(
        ...
        let aes_key = AesKey::new_decrypt(kek)
            .map_err(|_| CryptoError::DecryptFailed { message: "AES key unwrap: invalid KEK".into() })?;
        let mut out = vec![0u8; wrapped_key.len()]; // unwrap result is shorter
    ...
    fn rsa_attribute(
        ...
        // Sensitive attributes on the private-key object.
        if is_private && matches!(attr, AttributeType::Value) {
            return Err(CryptoError::AttributeSensitive);
        }

        // Parse key — extract n, e, and size from whichever half we have.
    ...
    fn ec_attribute(
        ...
        // Parse key to extract group + public point.
    ...
    fn aes_attribute(
        ...
        match attr {
            AttributeType::ValueLen => Ok(AttributeValue::Ulong(key.len() as u64)),
            // Value (raw key bytes) — the engine returns it; the PKCS#11 layer
            // applies CKA_SENSITIVE / CKA_EXTRACTABLE policy before calling this.
            AttributeType::Value    => Ok(AttributeValue::Bytes(key.to_vec())),
            _ => Err(CryptoError::AttributeTypeInvalid),
        }
    }
    ...
    fn ed_attribute(
        ...
        // Parse the key to determine which Edwards curve and get raw public key
    ...
    fn generate_chacha20_key(&self) -> Result<Vec<u8>, CryptoError> {
        let mut key = vec![0u8; 32]; // ChaCha20 always uses 256-bit keys
        rand_bytes(&mut key).map_err(key_err)?;
        Ok(key)
    }
    ...
    fn chacha20_poly1305_decrypt(
        ...
        let cipher = Cipher::chacha20_poly1305();
        decrypt_aead(cipher, key, Some(nonce), aad, ciphertext, tag).map_err(decrypt_err)
    }
